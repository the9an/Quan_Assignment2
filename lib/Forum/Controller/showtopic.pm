...
sub post_new_thread :Local :Args(0) 
{
    ...
    if ($c->req->params->{post_body}){
        ...
        my $newthread = eval { $threads_rs->create({
		    thread_content => thread_escape_content(thread_filter_content ($c->req->params->{thread_body})),
		    topic_id => $topic_id, 
...
sub thread_escape_content {
	my $body = $_[0];
	$body =~ s/\n/<p\/><p>/;
	$body =~ s{(https?://([-\w\.]+)+(:\d+)?(/([\w/_\.]*(\?\S+)?)?)?)}{<a href="$1">$1</a>}gx;
	$body =~ s{(\S+@\S+\.\S+)}{<a href="mailto:$1">$1</a>}gx;
	return '<p>'. $body. '</p>';
}
